# VSCode のインストールと Hula への接続

VSCode（Visual Studio Code）は、マイクロソフトが開発した無料のコードエディターです。動作が軽く、豊富な拡張機能を追加することで、さまざまなプログラミング言語や開発環境に対応させることができます。

このページでは、Windows 版 VSCode のインストールから、日本語化・Python 環境の準備、そしてドローン「Hula」への接続までを順を追って説明します。

## インストーラーのダウンロード

まず、公式サイトにアクセスし、「Download for Windows」ボタンからインストーラーをダウンロードします。

公式サイト: https://code.visualstudio.com/

![vscode1](./image/vscode1.png)

## インストーラーの実行

ダウンロードした `***.exe` ファイルを保存し、ダブルクリックして実行します。

![vscode2](./image/vscode2.png)

## ライセンスへの同意

使用許諾契約書（ライセンス事項）の内容を確認し、「同意する」を選択して「次へ」を押します。

![vscode3](./image/vscode3.png)

## インストール設定

基本的にはそのまま「次へ」を押して進めます。必要に応じて、インストール先のフォルダーやショートカットの作成などをカスタマイズすることもできます。

![vscode4](./image/vscode4.png)

## インストールの完了

設定内容を確認したら「インストール」を押し、処理が完了するまで待ちます。完了したら「完了」を押してインストーラーを閉じます。

![vscode5](./image/vscode5.png)

## 日本語化（拡張機能の導入）

VSCode は拡張機能で日本語化できます。左側の「拡張機能（Extensions）」アイコンを押し、検索欄に「japanese」と入力してください。表示されたマイクロソフト製の日本語化拡張機能（Japanese Language Pack）をインストールします。

導入したあとは、VSCode を再起動すると日本語表示が反映されます。

![vscode6](./image/vscode6-1.png)

## Python 拡張機能の導入

続いて、Python を扱うための拡張機能を導入します。同じく「拡張機能」の検索欄に「Python」と入力し、マイクロソフト製の Python 拡張機能をインストールしてください。

![vscode7](./image/vscode7-1.png)

## 作業フォルダーの作成

VSCode はフォルダーを指定して作業することができます。ここでは例として、デスクトップに「drone」というフォルダーを作成し、その中に「test.py」というファイルを作成します。

まず、デスクトップ上で右クリックし、「新規作成」→「フォルダー」を選択してください。

![makefile](./image/makefile.png)

フォルダーの名前は「drone」に設定してください。

![makefile](./image/filedrone.png)

次に、VSCode の「ファイル」メニューから「フォルダーを開く...」を選択してください。

![vscode8](./image/vsocdefolder1.png)

先ほど作成した「drone」フォルダーを選択してください。

![vscode8](./image/vsocdefolder2-1.png)

## 新規ファイルの作成

VSCode では、さまざまな方法でファイルを新規に作成できますが、作業フォルダーを指定しているときは、VSCode のエクスプローラーから新規作成するのが便利です。

「エクスプローラー」の「新しいファイル...」を押してください。ファイル名は「test.py」としてください。

![vscode9](./image/testpy1.png)

ファイルを作成したあと、以下のコードをコピーして「test.py」に貼り付けてください。

```python
import pyhula

api = pyhula.UserApi()
if not api.connect():
    print("connect error")
else:
    print("connection to station by wifi")
```

貼り付けると、以下のような画面が表示されます。

![vscode9](./image/testpy2.png)

## Hula への Wi-Fi 接続と確認
まずHula 本体に電源を入れてください。その後にHulaから電波を出すことができます。

Hula 本体の裏面に SSID が記載されています。パソコンの Wi-Fi 設定から、その SSID に接続してください。

パスワードは「12345678」です。

![vscode9](./image/vscode9.png)

Wi-Fi の接続が完了したら、再生マークの実行ボタンを押してください。
Pythonの通信をファイアウォールで許可するには、ポップアップ画面で「アクセスを許可する」をクリックしてください。
Hula への接続に成功すると、VSCode 下部の出力（ターミナル）部分に「connection to station by wifi」と表示されます。

![vscode10](./image/vscodelast.png)

以上で、VSCode の設定と Hula への接続は完了です。


## トラブルシューティング
## ウィルス対策ソフト

ノートPCからHula（機体のWi‑Fi）へ接続する場合、PCにインストールされているウイルス対策ソフト（セキュリティソフト）のファイアウォール機能が、SDK（pyhula）と機体との通信を遮断し、接続に失敗することがあります。接続がうまくいかないときは、下記の製品などが動作していないかを確認し、一時的に無効化するか、Python（python.exe）およびpyhulaの通信を許可する例外設定を行ってください。

問題がなければ、アンイストールすることをおすすめします。

代表的なウイルス対策ソフト：

- ウイルスバスター クラウド（トレンドマイクロ）
- ノートン 360（Norton）
- マカフィー リブセーフ（McAfee）
- E- SET インターネット セキュリティ
- カスペルスキー（Kaspersky）
- Avast / AVG


## ファイアウォール
WindowsでPythonの通信をファイアウォールで許可するには、ポップアップ画面で「アクセスを許可する」をクリックしなかった場合は、
pythonでHulaに接続できなくなります。

その場合は、「設定」→　「プライバシーとセキュリティ」→　「Windowsでのセキュリティ」→「ファイアウォールとネットワークの保護」→「ファイアウォールによるアプリケーションの許可」→「設定変更」→「Python」の左の欄にチェックしてください。

## その他問題がないとき
HulaのメソッドがIPアドレスをうまく取得できないときがあります。
Wi-Fiのプロパティからアドレスを確認してつながっているIPアドレスを確認してください。
connectの引数にIPアドレスを書くことで接続することができるときもあります。

```python
import pyhula

api = pyhula.UserApi()
if not api.connect(192.168.100.132):
    print("connect error")
else:
    print("connection to station by wifi")
```


